...
def plot_zxcorr_pairs(A,B,maxlag,plottag=''):
    
    fig, axs = plt.subplots(2,2,figsize=(7,7))
    axs[0,0].plot(A)
    axs[1,0].plot(B)
    axs[0,0].set_title('Signal segments')
    axs[0,0].set_ylabel('Signal A segment')
    axs[1,0].set_ylabel('Signal B segment')
    ...
    if len(plottag)>0:
        ...
        axs[0,0].set_title(tag)
        plt.savefig(plottag + 'ZXCORR_Pair.png', bbox_inches = 'tight')
    plt.show()
